    ...
    for ln in prog.lines:
        ...
        if mnem == 'push':
            ...
            if mnem == 'text':
                ...
                for a in args[:3]:
                    code += bytes([OP['push']]) + struct.pack('<h', _int_or_err(a, ln.lineno, 'text'))
                ...
                code += bytes([OP['push']]) + struct.pack('<h', _int_or_err(args[1], ln.lineno, 'load'))
                code += bytes([op]) + struct.pack('<H', str_off(prog, args[0]))
        else:
            ...
            for a in args:
                code += bytes([OP['push']]) + struct.pack('<h', _int_or_err(a, ln.lineno, mnem))
            code += bytes([op])
...
def main() -> None:
    ...
    try:
        ...
        sys.exit(1)
    print(f'OK: {out_path} ({len(blob)} bytes) code={blob[6] | (blob[7] << 8)} '
